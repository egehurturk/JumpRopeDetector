...
class RopeDetectionController {
    ...
    func setupVision() {
        do {
            ...
            let ropeRecognition = VNCoreMLRequest(model: visionModel, completionHandler: { (request, error) in
            ...
            ropeRecognition.imageCropAndScaleOption = .scaleFill
            self.requests = [ropeRecognition]
...
//        shapeLayer.borderWidth = 12.0
    ...
    func drawVisionRequestResults(results: [Any]) {
        ...
        for observation in results where observation is VNRecognizedObjectObservation {
            ...
            // must ensure that we detect people
            if (topLabelObservation.identifier != "person" || topLabelObservation.confidence < self.confidenceThreshold) {
                continue
            }
            
            print("coordinates of human: ", objectBounds)
    ...
    func countJumps(pixelBuffer: CVPixelBuffer) {
